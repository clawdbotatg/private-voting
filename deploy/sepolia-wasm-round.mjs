...
const cts = VOTES.map(choice => {
  const v = new Int32Array(OPTIONS)
  v[choice] = 1
  return fhe.encrypt_vector(params, publicKey, v)
})
console.log(`✓ ${cts.length} ballots encrypted with weft-wasm (${cts[0].length} bytes each)`)
