# private-voting — slop computer × The Interfold

Private voting for [live.slop.computer](https://live.slop.computer) on
the real [Interfold](https://theinterfold.com) protocol: encrypted
ballots published on-chain, homomorphically tallied, threshold-decrypted
by a ciphernode committee. Phase 1 (in-browser threshold BFV demo)
lives in the slop-computer-live repo's Voting Booth app; this repo is
phase 2 — the same votes running through real E3s, first locally, then
against the Sepolia testnet committee.

Scaffolded with `interfold init` (upstream template docs below).
Local end-to-end round passes: `PORT=8080 pnpm test:integration`
(sortition → distributed DKG across 5 ciphernodes → encrypted inputs
on-chain → FHE sum → threshold decrypt → correct plaintext).

Changes vs the stock template:
- `server/index.ts`: retry `publishCiphertextOutput` on
  `InputDeadlineNotReached` — the template publishes on a wall-clock
  timer but the contract enforces the chain clock, and one early
  attempt killed the whole round.
- `Cargo.lock`: regenerated (template shipped stale vs its pinned
  interfold rev) with `alloy-sol-type-parser` pinned to 1.5.4.

Plan + research: `ops/PLAN-private-voting.md` in
